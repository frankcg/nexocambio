...
def text(x, y, s, size=13, bold=False, fill="#232F3E", anchor="start", italic=False):
    a(f'<text x="{x}" y="{y}" font-family="{FONT}" font-size="{size}" fill="{fill}" text-anchor="{anchor}"'
      f'{" font-weight=\"700\"" if bold else ""}{" font-style=\"italic\"" if italic else ""}>{esc(s)}</text>')
...
# Frontend + API
box(225, 150, 230, 118, "s3", "S3", "S3 — Sitio web", ["Frontend estático", "(HTML/CSS/JS responsive)", "Landing · cotizador · login", "registro · mis operaciones"], tsize=14)
box(225, 385, 230, 130, "api", "API", "API Gateway", ["HTTP API · stage v1", "CORS · throttling", "9 rutas → 3 Lambdas"], tsize=14)
flecha("M112,395 L112,210 L225,210", label="① HTTPS (HTML/JS)", lx=150, ly=200)
flecha("M160,460 L225,450", label="② HTTPS + JSON + JWT", lx=192, ly=548)
...
box(585, 385, 240, 130, "lambda", "λ", "Operaciones", ["POST/GET /operaciones", "POST …/comprobante", "PATCH …/estado (back-office)"])
...
# API → Lambdas
flecha("M455,450 L520,450 L520,234 L585,234", label="③", lx=508, ly=340)
flecha("M455,450 L585,450", marker=True)
flecha("M455,450 L520,450 L520,659 L585,659")
# Invocación interna
flecha("M705,385 L705,293", "#B45309", label="Lambda Invoke: obtener cotización", lx=705, ly=343, w=2.2)
...
box(985, 372, 250, 76, "ddb", "DB", "nexo-operaciones", ["PK id_operacion", "GSI cliente-fecha-index"], tsize=13.5)
box(985, 462, 250, 76, "s3", "S3", "S3 comprobantes", ["Privado · cifrado AES-256", "URL temporal firmada"], tsize=13.5)
...
flecha("M705,175 L705,108 L1422,108 L1422,150", "#5B6B85", dash=True, label="HTTPS (opcional; con caché en DynamoDB)", lx=1060, ly=100)
...
flecha("M340,515 L340,880 L985,880 L985,850", "#E7157B", dash=True, label="métricas de la API", lx=660, ly=872)

# Notas
a('<rect x="1290" y="300" width="265" height="410" rx="10" fill="#F0F5FF" stroke="#3B48CC" stroke-width="1.2"/>')
...
notas = ["• Cada microservicio tiene su", "  propia base de datos.", "• Operaciones NO confía en la tasa", "  del navegador: la valida con", "  Cotizaciones (Lambda→Lambda).",
         "• Contraseñas con PBKDF2;", "  sesión con JWT (2 h).", "• Comprobantes: bucket privado,", "  validación de formato real y", "  enlaces temporales.",
         "• Pago por uso: sin servidores", "  encendidos 24×7.", "• Frontend en S3 (HTTP); con", "  CloudFront se obtiene HTTPS."]
for i, n in enumerate(notas):
    text(1305, 348 + i * 22, n, 12, fill="#232F3E")

# Opcional
a('<rect x="20" y="640" width="150" height="230" rx="10" fill="#fff" stroke="#8A94A6" stroke-width="1.4" stroke-dasharray="6 5"/>')
text(95, 664, "Opcional si el Lab", 11.5, True, "#5B6B85", "middle"); text(95, 680, "lo habilita:", 11.5, True, "#5B6B85", "middle")
for i, l in enumerate(["CloudFront (HTTPS", "y caché del sitio)", "", "Cognito (usuarios", "en vez de JWT propio)", "", "EventBridge + SES", "(avisos de estado)"]):
    text(30, 708 + i * 17, l, 11, fill="#4A5568")
text(W / 2, 950, "Flujo MVP: cotizar → registrarse / iniciar sesión → registrar operación → adjuntar comprobante → consultar estado", 13, True, "#0E1B33", "middle")
a('</svg>')
